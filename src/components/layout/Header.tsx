...
export function Header({ title, showBack = true, onBackClick }: HeaderProps) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-ink/98 border-b border-ash backdrop-blur-sm">
        ...
        <div className="flex items-center gap-3 flex-1">
          {shouldShowBack && (
            <Button
              ...
              size="sm"
              onClick={handleBack}
              className="text-bone hover:text-ember transition-colors -ml-2"
              aria-label="Go back"
            >
              <ChevronLeft size={24} />
            </Button>
          )}
          <h1 className="font-display text-lg font-bold text-bone tracking-[0.15em]">
            {title || "ELS"}
          </h1>
        </div>
        <AuthPill />
